...
plus a per-(client, radiod) accumulator that resolves hashes from
``<call>`` announcement markers in decoded text.
...
Public API:
    ...
    from sigmond.callhash import CallHashTable, hash22, hash15, nhash

    table = CallHashTable.load_or_new("/var/lib/<client>/<radiod>/hashtable.json")
...
``tests/test_callhash_nhash.py`` (including 11/12/13-byte boundary
cases that exercise distinct final-block branches).
"""
from ._nhash import (
    MASK10,
    MASK12,
