...
PYEOF
        ...
        # radiod-side upward pull.  PartOf= on the consumers propagates only
        # radiod's stop and restart -- a plain `systemctl start radiod` leaves
        # every consumer down.  That is not hypothetical: on AC0G-B4
        # 2026-08-30 the radiod watchdog stopped radiod (taking the consumers
        # with it, correctly) and then failed to start it.  Starting radiod by
        # hand brought back radiod alone; the whole recorder set had to be
        # started one unit at a time.  Wants= closes the other half of the
        # loop, so the pair is complete: Wants= pulls the consumers up with
        # radiod, PartOf= takes them down with it.
        if (( ${#_CONSUMERS[@]} )); then
            $SUDO install -d -m 0755 "/etc/systemd/system/${_RADIOD_UNIT}.d"
            {
                echo "# Generated by sigmond install.sh from [radiod].consumers"
                echo "# in ${_TOPOLOGY}.  Starting radiod pulls these up; their"
                echo "# own PartOf= drop-in takes them back down with it."
                echo "[Unit]"
                for _c in "${_CONSUMERS[@]}"; do echo "Wants=${_c}"; done
            } | $SUDO tee "/etc/systemd/system/${_RADIOD_UNIT}.d/20-sigmond-consumers.conf" >/dev/null
