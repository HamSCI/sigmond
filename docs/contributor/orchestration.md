...
| `adopt` | after attaching hardware | adopt one offer named by `smd status` — record its sources against the components it brings, and name the install/start that follows; `--dry-run` reports the plan and changes nothing. The only verb in the adoption design that starts anything: detection alone never does. Distinct from `admin manifest adopt`, which adopts a blessed image manifest | `cmd_adopt` → `adoption.py` (`offers`, `plan`), `station_identity.py` (`identify`), `sources.py` (`ClientSources`) | yes (root), except `--dry-run` |
