...
# --- chronyc sources/tracking parsers + formatters (timing.py) -----------
#
# Promoted from ``screens/timing.py`` alongside ``chrony_tracking()`` above:
# these are pure parsers/formatters with no Textual dependency, but they
# used to live in a module that imports ``textual`` at the top, which
# dragged Textual into every import (including from tests that only want
# the parsing logic). ``screens/timing.py`` now imports these rather than
# keeping its own copy.

# Refresh cadence and history depth for the Timing screen's sparkline.  60
# samples at 1 Hz = the last minute of trace per source, which spans a
# full Costas excursion plus margin on each side.
HISTORY = 60
