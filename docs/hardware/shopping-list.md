...
| A small disk with retention left at defaults | The timing client writes about 6.7 GB of raw IQ per channel per day. The source calls **9 channels on a 120 GB disk** the absolute margin — it fills within a single day unless retention is set aggressively (source: `hf-timestd/INSTALLATION.md` §Storage Requirements). |
