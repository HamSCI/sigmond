...
| **installed** | Present on the station: the code is checked out and `smd version` prints its commit. Installed does **not** mean running and does not mean *enabled* — dasi002 listed 23 installed components while `smd status` showed 9 client blocks (live, 2026-08-23). |
