...
| `adopt` | after attaching hardware | adopt one offer named by `smd status`: record its sources for the components that consume them, then enable and start what the offer brings, after an explicit yes (`--yes` for scripted provisioning; an absent terminal is never read as consent). `--dry-run` reports the plan and starts nothing. The only verb in the adoption design that starts anything — detection alone never does. Distinct from `admin manifest adopt`, which adopts a blessed image manifest | `cmd_adopt` → `adoption.py` (`offers`, `plan`, `SOURCE_CONSUMERS`), `station_identity.py` (`identify`), `sources.py` (`ClientSources`), then `cmd_start` under `lifecycle_lock` | yes (root + lock), except `--dry-run` |
