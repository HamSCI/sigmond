...
Multi-phase initiatives get a design doc at `tasks/plan-<topic>.md` — one
per initiative, written and agreed before code lands. The existing set
(`plan-ch-wsprdaemon-removal`, `plan-local-resources-probe`,
`plan-multi-rx888-sources`, `plan-proxmox-vm-bootstrap`,
`plan-stream-quality-surface`) shows the shape: a **Status** line stating
what has shipped vs. what's deferred and why, the motivation, explicit
non-goals, the architectural choice with its rationale, and per-phase task
lists. They are living documents — update the Status line as phases land
rather than deleting the doc.

These plans are also the record of *why* a design was chosen over the
alternatives; check `tasks/` before relitigating an architectural decision.
