    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            yield Input(value=self._meta.label,
                        placeholder="e.g. sigma-status  →  sigma-status.local",
                        id="dm-label")
            yield Static("[dim]the full radiod status name; .local is appended "
                         "→ <name>.local (clients connect to this; "
                         "convention: <host>-status)[/]",
...
def _build_rx888_conf(config_name: str, device_index: int, ttl: int = 0) -> str:
    ...
    ``config_name`` is the radiod status multicast name *verbatim* — radiod
    publishes it as ``<config_name>.local`` and clients connect to that.  The
    operator owns the whole name (no forced ``-hf.status`` suffix); the data
    (PCM) group is derived as ``<base>-pcm.local`` where <base> drops a
    trailing ``-status`` so it never reads ``...-status-pcm``.
    """
    ttl_comment = "  # set to 1 to send multicast out ethernet" if ttl == 0 else ""
    data_base = config_name.removesuffix("-status")
    return (
        ...
        class AddRemoteModal(_MS):
            ...
            def compose(self):
                with Vertical():
                    ...
                    yield _In(placeholder="e.g. bee1-status.local  or  192.168.1.10",
                              id="ar-addr")
