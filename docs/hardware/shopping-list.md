...
> **Audience:** operator
> **Status:** current
> **Verified against:** sigmond fbc432f on 2026-08-23 — live b4 (lsusb, smd admin environment) + code/docs
...
antenna, and a network cable. The radio is an **RX888 Mk II** — it
digitises everything from 10 kHz to 64 MHz at once and fire-hoses it
over USB 3 at more than 2 Gbit/s (source:
`ka9q-radio/docs/SDR/rx888.md`), so the computer has to be a real
x86-64 machine with 16 GB of RAM and an internal NVMe drive, not a Pi
(source: `sigmond-appliance/INSTALL.md` §1). Everything else on this
page is optional and buys you one more science product: a
GPS-disciplined oscillator (*GPSDO*) for a clock the ionosphere cannot
push around, a **TS-1** time injector for nanosecond-class timing, an
**RM3100** magnetometer for geomagnetic data, a dual-frequency GNSS
receiver for total-electron-content work. A station with only the
required items already produces WSPR and FT8/FT4 spots. Budget roughly
**$650–1,300** for the required set and **$1,300–2,300** for the full
...
|---|---|---|---|
| Station computer | x86-64 mini-PC, **16 GB RAM or more**, internal NVMe/SSD. AC0G/B4 gives its station VM 14 vCPUs, ~9 GB RAM and a 252 GB disk (52 % used) (source: `nproc; free -g; df -h` on b4, 2026-08-23). Both reference stations are Beelink-class mini-PCs (source: `mag-recorder/docs/PROVENANCE.md`, `gpsdo-monitor/README.md` §Status). | The radio delivers over 2 Gbit/s and `radiod` must consume every block within 20 ms or samples are lost, so the machine needs real cores and real memory (source: `ka9q-radio/docs/SDR/rx888.md`; `sigmond/docs/PACKET-LOSS-DIAGNOSTICS.md`). | ⚠ **The whole internal drive is erased** by the appliance installer (source: `sigmond-appliance/INSTALL.md` §1). Disk size follows how many timing channels you record: 3 channels want 60 GB minimum, 6 channels 120 GB, 9 channels 180 GB, and more is better (source: `hf-timestd/INSTALLATION.md` §Storage Requirements). Not a laptop — see [Things that look right but aren't](#things-that-look-right-but-arent). |
...
Three things worth knowing before you buy the optional parts:
...
- ⚠ **The 1 PPS the GPSDO reports over USB is a liveness indicator, not
  ...
  `/run/gpsdo/0C7BB80D10EF.json` on b4, 2026-08-23). Real timing comes
  from the disciplined reference frequency into the radio and, at the
  top tier, from the TS-1 path.
...
- **GPSDO → RX888 reference input**: the LBE-1421 has two outputs and b4 runs both — OUT1 at 10 MHz and OUT2 at 27 MHz, both enabled (source: `/run/gpsdo/0C7BB80D10EF.json` on b4, 2026-08-23). The RX888's own sampling clock is 27 MHz, and ka9q-radio's guidance for a precise sampling clock is to feed an external 27 MHz reference to the connector inside the unit (source: `ka9q-radio/docs/SDR/rx888.md` §calibrate). Sigmond's own docs describe the GPSDO generically as "10 MHz + PPS" (source: `sigmond/docs/installation-guide.md`). ⚠ **Which jack each output is physically patched into is not recorded in any file on the station** — check your own radio's manual and label your cables. Tracked as a gap in [`../contributor/docs-gap-ledger.md`](../contributor/docs-gap-ledger.md).
- **GPSDO → station computer** over USB: **monitoring only.** This link reports lock state, GPS fix, satellite count and output frequencies; it carries no usable timing (source: `gpsdo-monitor/README.md`).
...
- **RM3100 → Pololu adapter** over I²C (addresses 0x23 and 0x1F), then **Pololu → station computer** over USB, appearing as `/dev/ttyMAG0` (source: `mag-recorder/README.md` §Data flow).
- **Station computer → router**: ordinary wired Ethernet.

A powered USB **hub is fine** for the slow devices — on b4 the GPSDO and the magnetometer adapter both sit behind VIA Labs USB 2.0 hubs (source: `lsusb`, 2026-08-23). It is only the RX888 that must go straight to the machine.
...
| **Required subtotal** | **~$650–1,300** | |
| Leo Bodnar LBE-1421 GPSDO + GPS antenna | $200–320 | optional |
| TS-1 time injector (Turn Island Systems) + GPS antenna | $250–400 | optional |
| PNI RM3100 magnetometer board | $50–120 | optional |
| Pololu Isolated USB-to-I²C adapter (5396/5397) | $35–50 | optional |
| u-blox ZED-F9P GNSS board + antenna | $200–350 | optional |
| **Full build (what B4 runs, minus the F9P)** | **~$1,300–2,300** | |
...
Two devices in b4's `lsusb` are **not** station parts: a QEMU Tablet and
...
| A small disk with retention left at defaults | The timing client writes about 6.7 GB of raw IQ per channel per day. Six channels on a 120 GB disk is already the margin (source: `hf-timestd/INSTALLATION.md` §Storage Requirements). |
