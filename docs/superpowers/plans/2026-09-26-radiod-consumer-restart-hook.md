...
- Stale = a consumer's earliest active unit's MONOTONIC start (`ActiveEnterTimestampMonotonic`) is before the named radiod unit's monotonic start plus `sigmond-radiod-ready`'s settle allowance (`sigmond.radiod_ready.READY_SETTLE_S`) — never wall-clock `ActiveEnterTimestamp` (final review / I-1: these stations step their host clock, and a wall-clock comparison would misjudge staleness across a step; a consumer can also start inside radiod's own settle window and take its anchor before radiod actually serves). Where consumer drop-ins declare `After=radiod` (and an `ExecStartPre` gate), consumers start after radiod plus its settle window, so nothing is stale at boot and the hook is a no-op there — AC0G-ND has no such drop-ins, which is the whole reason this rule exists.
...
- Lifecycle lock: wait for it (poll every 5 s, up to 900 s) rather than fail; re-read staleness AFTER acquiring it (an `smd align` that just restarted radiod restarts its consumers itself — afterwards nothing is stale). Lock still busy at the deadline → exit 3, restart nothing. `_wait_lifecycle_lock` narrates a wait: one line when first found busy, one when acquired (final review / M-2). Final review / R2: the 900 s is a single RUN-WIDE budget shared by every pass, not reset each pass — an absolute deadline computed once, before the first pass, and handed unchanged to every later pass. Without this, 3 independent 900 s waits could together blow past the unit's `TimeoutStartSec`. A pass that finds the shared budget already spent by an earlier pass returns 3 without even trying the lock.
- final review / I-2: radiod can restart again WHILE this run restarts its own consumers (ND's RX888 stalls repeat) — that restart's own hook invocation folds into this still-running job and never fires on its own. After a pass, re-read radiod's monotonic start; a change reruns from the readiness check, up to 3 passes total; the 3rd pass still changing → exit code at least 1. Final review / R1: a recheck that reads `None` (radiod not currently active — inside its own start job, in ExecStartPost, or crashed again mid-pass) counts as CHANGED too, exactly like a different monotonic value, never as "unchanged, done" — that pass's own readiness wait then decides what it can do.
- final review / I-4: the catalog is loaded by the hook itself, once; a load failure restarts nothing and exits 4 (never falls back to "every running service is a consumer", which `_align_staleness`'s own best-effort default does and which would restart sigmond-rac, gpsdo-monitor, station-web, unattended). Every consumer considered is printed on every run, dry or not: its label, its monotonic offset from radiod's start, and its verdict (stale / fresh / not running).
- `--dry-run` prints what would restart and changes nothing.
- Exit codes: 0 nothing stale or all restarted; 1 a restart failed; 2 radiod not ready / unit unknown; 3 lock not obtained; 4 catalog did not load. Final review / R3: a run of several passes returns the WORST code seen across all of them, by precedence 4 > 3 > 2 > 1 > 0 (the codes are already numbered in ascending severity, so this is `max()` over every pass's rc) — a failed restart in an early pass is never hidden by a later, clean pass.
