...
- Stale = a consumer's earliest active unit `ActiveEnterTimestamp` is BEFORE the named radiod unit's `ActiveEnterTimestamp`. At boot, where consumer drop-ins declare `After=radiod` (and an `ExecStartPre` gate), consumers start after radiod, so nothing is stale and the hook is a no-op — final review / I-1: on a station with no such drop-ins (AC0G-ND), a consumer can start inside radiod's own settle window and take its anchor before radiod actually serves, so staleness compares on the MONOTONIC clock against radiod's start plus `sigmond-radiod-ready`'s settle allowance (`sigmond.radiod_ready.READY_SETTLE_S`), not wall-clock `ActiveEnterTimestamp`.
...
- Lifecycle lock: wait for it (poll every 5 s, up to 900 s — final review / M-4: the verb's default, sized to cover 3 passes' readiness waits plus the restarts themselves) rather than fail; re-read staleness AFTER acquiring it (an `smd align` that just restarted radiod restarts its consumers itself — afterwards nothing is stale). Lock still busy at the deadline → exit 3, restart nothing. `_wait_lifecycle_lock` narrates a wait: one line when first found busy, one when acquired (final review / M-2).
- final review / I-2: radiod can restart again WHILE this run restarts its own consumers (ND's RX888 stalls repeat) — that restart's own hook invocation folds into this still-running job and never fires on its own. After a pass, re-read radiod's monotonic start; a change reruns from the readiness check, up to 3 passes total; the 3rd pass still changing → exit 1.
